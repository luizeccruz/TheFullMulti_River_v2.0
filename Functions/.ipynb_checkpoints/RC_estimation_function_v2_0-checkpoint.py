...
def RC_estimation_function_v2_0(processList, CombList, Clist, MP_prop, compartments_prop, process_df, numberRS, composition, mode2, mode, date, riverComp, MPforms, sizeBin, river_flows, settling_method, sphericity, reynolds,water_type):
    ...
    # if len(Clist) <= 10*len(riverComp)*len(MPforms)*len(sizeBin):
        for spc in Clist:
            ...
            MP_index = np.where((MP_prop['composition'] == composition) & (
                MP_prop['sizeBin'] == sizeBinIdx))[0][0]
            SPM_index = np.where((MP_prop['name'] == "SPM1") & (
                MP_prop['sizeBin'] == sizeBinIdx))[0][0]
            comp_index = np.where((compartments_prop['compartment'] == int(compartment)) & (
                compartments_prop['riverSection'] == int(riverSection)))[0][0]
            rs_indexs = np.where(
                compartments_prop['riverSection'] == int(riverSection))[0]
            # for processes
            idx = np.where((process_df['aggState'] == aggState) & (
                process_df['compartment'] == int(compartment)))[0][0]

            # Generate compartment objects corresponding to the River Section
            [surface, flowingWater, stagnantWater, sediment] = preProcessLayers(
                mode, compartments_prop, date, rs_indexs, riverSection)
            # Create a dictionary with the environmental compartments objects
            comp_dict = {"1": surface, "2": flowingWater,
                         "3": stagnantWater, "4": sediment}
            ...
            RC_df.loc["settling", spc[2:6]] = settling(
                particle, surface.depth_m, settling_method, compartment, water_type)
            RC_df.loc["rising", spc[2:6]] = rising(
                particle, flowingWater.depth_m, settling_method, compartment, water_type)
            ...
            list_settling.append(settling(particle, surface.depth_m, settling_method, compartment, water_type))
            list_rising.append(rising(particle, flowingWater.depth_m, settling_method, compartment, water_type))
        ...
        for x in RC_df:
            for y in range(len(RC_df)-2):
                if type(RC_df[x][y]) is tuple:
                    if RC_df.index[y] == "fragmentation":
                        ...
                        # if its a tuple but not the fragmentation process it means there are several values for this parameter and the timeLimit has to be applied to every value
                        RC_df_templim[x][y] = tuple(
                            [0.000556 if i > 0.000556 else i for i in RC_df[x][y]])

                elif type(RC_df[x][y]) == list:
                    lis = RC_df[x][y]
                    RC_df_templim[x][y] = [0.000556 if i >
                                           0.000556 else i for i in lis]
                else:
